...
| β-VAE揭示了可调节的语义信息率 | β=0.01, LD=8: MSE=0.0474, KL码率=13.2 bits; KL跨度300倍（19.6→0.06 nats） | 对语义通信至关重要的一点：码率由信息瓶颈度量 | 表2，图 ablation_heatmap |
| β≥0.5时KL触及free-bits地板（有效坍塌） | KL≈0.06–0.09 nats (~0.1 bits), MSE→0.55（数据方差）; free_bits=0.01允许宽动态范围 | SemCom-MARL训练监测：KL触及地板 = < 0.1有效bits | 表2，图 kl_collapse |
| VQ-VAE在LD=4下表现最佳 | 最佳: CB=512, LD=4, 9 bits, MSE=0.1283, 码本利用率100% | 需要离散符号信道载荷时的替代方案 | 表3，图 vqvae_usage_heatmap |
...
| VQ-VAE | 离散码本瓶颈 | `codebook_size × latent_dim × commitment_cost` | 27个独立运行 |
...
**数字量化在纯重建任务上占据主导地位；β-VAE则刻画了信息瓶颈前沿。** 当重建是唯一目标且可用128以上名义bits时，数字基线达到最佳MSE。β-VAE对语义通信至关重要，因为它通过KL散度度量了**有效信息率**，这使我们得以研究潜在信道中语义空洞发生的位置。使用free_bits=0.01 nats/dim后，KL在β=0.001到β=0.5之间跨越300倍动态范围，之后MSE如理论预测般逼近数据方差（~0.545）。（数据来源：表1、表2。）
...
**KL跨越300倍动态范围，在β≥0.5时触及free-bits地板。**
在修正后的架构（平衡的编码器-解码器容量、BatchNorm编码器、50 epoch KL预热、free-bits=0.01 nats/dim）下，后验分布永远不会坍塌至零KL。从β=0.001（KL≈19.6 nats, 28 bits）到β=0.5（KL≈0.09 nats, 0.1 bits），KL平滑下降跨越两个数量级。β≥0.5后，KL触及逐维free-bits地板，MSE逼近数据方差（~0.545），与β→∞的理论极限一致。这相比旧架构（无free bits和预热，KL在β≥0.5时坍塌至<10⁻⁴）和中间方案fb=0.1（在所有高β下人为抬高KL至~0.7 nats）都是显著改进。修正后的行为与Higgins et al. (2017)和Burgess et al. (2018)一致。（数据来源：表2。）
...
**VQ-VAE在高潜在维度下码本严重过度配置。**
对于CB=256和LD=8，无论commitment cost如何变化，码本利用率始终低于12%——
离散潜在空间对该18维MPE观测分布过度配置。VQ-VAE最佳点（CB=512, LD=4, cc=0.25）
达到MSE=0.1283（9 bits），码本利用率100%。在LD=2设置下，码本利用率在所有码本
大小下均达100%，其中CB=256达到MSE=0.1762（8 bits）。
低维离散化在该数据上既更高效也更为稳定。（数据来源：表3。）
...
| β范围 | 区间 | KL / 码率行为 | 用途 |
|---------|--------|--------------------|-----|
| β=0.001 | 高码率近AE区 | KL≈19.6 nats, 码率≈28 bits, MSE≈0.035 | 重建参考基线 |
| β=0.01 | 语义瓶颈区 | KL≈9.2 nats, 码率≈13 bits, MSE≈0.047 | 推荐的SemCom-MARL探针 |
| β=0.1 | 过渡区 | KL≈1.3 nats, 码率≈1.9 bits, MSE≈0.211 | 边界压力测试 |
| β=0.2–0.3 | 低码率区 | KL≈0.3–0.7 nats, 码率≈0.4–1.0 bits | 最低限度信息 |
| β≥0.5 | KL地板区 | KL≈0.06–0.09 nats, 码率≈0.1 bits, MSE→0.55 | 近乎完全坍塌；KL在free-bits地板 |

## 负面结果及其方法论价值

本基准测试中的两项负面结果为未来的SemCom-MARL工作承载了重要的方法论价值：

1. **Free bits阻止了零KL坍塌，但β≥0.5时仍发生有效坍塌。**
   在修正后的架构（free-bits=0.01 nats/dim、KL预热、平衡的编码器-解码器）下，后验分布永远不会坍塌至零KL。KL从β=0.001（KL≈19.6 nats, 28 bits）到β=0.5（KL≈0.09 nats, 0.1 bits）跨越300倍动态范围，之后触及free-bits地板（0.01 × LD nats）。β≥0.5时，KL稳定在~0.06–0.07 nats，MSE逼近数据方差（~0.545），与β→∞的理论极限一致。此前的free_bits=0.1设置在β≥0.5时人为抬高KL至~0.7 nats，掩盖了真实的坍塌行为。这提供了一个实用的监测阈值：**当SemCom-MARL训练中KL触及free-bits地板时，潜在信道携带的任务相关信息可忽略不计（< 0.1有效bits）。**

2. **VQ-VAE在高潜在维度下码本利用率坍塌。**
   LD=8且CB=256时，在所有测试的commitment cost下，码本利用率从未超过12%。即使在最小码本（CB=32）下，LD=8的最高利用率也仅为43.8%。这并非训练失败——它表明离散潜在空间对仅有18维的MPE观测（模态多样性有限）存在结构性过度配置。实践启示：**在该数据分布上使用LD≤4进行离散语义信道编码；LD≥8仅为连续（β-VAE）瓶颈保留。** 最佳VQ-VAE结果（CB=512, LD=4, MSE=0.1283, 9 bits, 100%利用率）证实中等潜在维度配合大码本是性能甜点。
