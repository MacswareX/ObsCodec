...
ObsCodec 追问一个简洁的问题：**机器人的观测信号在丢失任务相关信息之前，可以被压缩到什么程度？**
...
| β-VAE暴露了可调节的语义信息率 | β=0.01, LD=8: MSE=0.0873, KL码率=6.4 bits | 对语义通信至关重要：码率由信息瓶颈度量 | 表2，图 ablation_heatmap |
| β≥0.5导致后验坍塌 | KL≈0，MSE≈0.545，跨越所有潜在维度 | 为SemCom-MARL训练中的坍塌监测提供了具体阈值 | 表2，图 kl_collapse |
...
多机器人系统经常在通信受限条件下运行：水下机器人、灾害响应编队、仓储车队以及对抗性或低带宽的野外环境。原始观测的完整共享是低效的——语义通信应传输有助于智能体协调的信息。

ObsCodec是将编解码器集成到完整MARL回路之前的前置研究。它隔离了观测压缩问题，使率失真权衡在加入策略学习之前变得可见可测。

因此，本项目是一个聚焦的研究原型，面向：

- **语义通信**：β-VAE通过KL散度提供了显式的信息率度量
- **多智能体系统**：数据来源于多智能体粒子世界观测
- **具身智能**：信号是类机器人的观测向量，而非静态图像基准
...
**数字量化在纯重建任务上占据主导地位；β-VAE则刻画了信息瓶颈前沿。** 当重建是唯一目标且可用128+名义bits时，数字基线达到最佳MSE。β-VAE对语义通信至关重要，因为它通过KL散度度量了**有效信息率**，使我们得以研究潜在信道在何处变得语义空洞。（数据来源：表1、表2。）
...
**该前沿是带宽约束下codec选型的设计地图。** 数字量化适用于高保真观测回放；β-VAE是信息瓶颈研究的工具，在有效码率比原始MSE更重要的场景下使用；VQ-VAE适用于离散、低比特率信道接口比重建精度更重要的场景。（数据来源：表1、表4。）
...
**后验坍塌在β≥0.5时急剧发生——一个可复现的失效边界。** 在此阈值下，KL在整个(LD, β)网格中降至0.05 nats以下，重建MSE饱和在约0.545——即先验N(0,I)的方差。编码器停止携带与输入相关的信息；潜在信道退化为先验分布。该边界在所有测试的潜在维度（LD=2至32）上一致成立。（数据来源：表2。）
