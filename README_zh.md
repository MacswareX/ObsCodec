...
| β-VAE揭示了可调节的语义信息率 | β=0.01, LD=8: MSE=0.0478, KL码率=13.3 bits | 对语义通信至关重要的一点：码率由信息瓶颈度量 | 表2，图 ablation_heatmap |
| β≥0.5时KL触及free-bits地板（有效坍塌） | KL≈0.7 nats (~1 bit), MSE≈0.35; free bits阻止坍塌至零KL | 为SemCom-MARL训练中的坍塌监测提供了具体阈值 | 表2，图 kl_collapse |
...
**数字量化在纯重建任务上占据主导地位；β-VAE则刻画了信息瓶颈前沿。** 当重建是唯一目标且可用128以上名义bits时，数字基线达到最佳MSE。β-VAE对语义通信至关重要，因为它通过KL散度度量了**有效信息率**，这使我们得以研究潜在信道中语义空洞发生的位置。在修正后的编码器-解码器平衡下，KL在β≥0.5时触及free-bits地板（~0.1 nats/dim），防止坍塌至零KL，但编码器仅保留最低限度的信息。（数据来源：表1、表2。）
...
**KL在β≥0.5时触及free-bits地板——防止零KL坍塌。**
在修正后的架构（平衡的编码器-解码器容量、BatchNorm编码器、50 epoch KL预热、free-bits=0.1 nats/dim）下，后验分布永远不会坍塌至零KL。在β≥0.5时，KL收敛至~0.7 nats（约1个有效bit）——即LD=8的free-bits地板——重建MSE饱和在0.34–0.36左右。编码器保留了最低限度但非零的信息容量。相比旧架构（无free bits和预热，KL在β≥0.5时坍塌至<10⁻⁴），这是显著改进。修正后的行为与Higgins et al. (2017)和Burgess et al. (2018)一致。（数据来源：表2。）
...
| β范围 | 区间 | KL / 码率行为 | 用途 |
|---------|--------|--------------------|-----|
| β=0.001 | 高码率近AE区 | KL≈19.5 nats, 码率≈28 bits, MSE≈0.036 | 重建参考基线 |
| β=0.01 | 语义瓶颈区 | KL≈9.2 nats, 码率≈13 bits, MSE≈0.048 | 推荐的SemCom-MARL探针 |
| β=0.1 | 过渡区 | KL≈1.7 nats, 码率≈2.5 bits, MSE≈0.183 | 边界压力测试 |
| β=0.2–0.3 | 低码率区 | KL≈0.8–1.1 nats, 码率≈1.1–1.6 bits | 最低限度信息 |
| β≥0.5 | KL地板区 | KL≈0.7 nats, 码率≈1 bit, MSE≈0.34–0.36 | 有效坍塌；free bits阻止零KL |
...
1. **Free bits阻止了零KL坍塌，但β≥0.5时仍发生有效坍塌。**
   在修正后的架构（free-bits=0.1 nats/dim、KL预热、平衡的编码器-解码器）下，后验分布永远不会坍塌至零KL——KL收敛至free-bits地板（LD=8时约0.7 nats），而非旧架构中的<10⁻⁴。然而，在此地板水平下，编码器携带的信息极少：有效码率约1 bit，MSE饱和在0.34–0.36。这提供了一个实用的监测阈值：**当SemCom-MARL训练中KL触及free-bits地板时，潜在信道携带的任务相关信息可忽略不计。** 架构修正将坍塌起始点从β=0.5（旧架构，零KL）变为β≥0.5（修正后，触及地板），但实际后果——潜在表示不可用——是相同的。这证实了Alemi et al. (2018)的率失真框架在非图像（机器人观测）数据上仍然准确预测了坍塌行为。
