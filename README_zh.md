...
**Route B** 扩展了Phase 1基准，涵盖激进维度扩展、通过free-bits实现的通用后验坍塌预防、
信道损伤鲁棒性测试，以及跨场景泛化——共计263个训练模型。

| 发现 | 证据 | 重要性 | 详见图表 |
|------|------|--------|----------|
| FB=0.1通用预防后验坍塌 | 全场景0%坍塌率（18-90维，3-15智能体） | 单一free-bits值适用于所有场景——无需逐场景调参 | 表2, 图collapse_barrier |
| 最小有效FB剂量=0.02 nats/dim | FB=0.02时KL=0.31 nats，MSE单调改善至FB=0.25 | 比文献常用值(0.5-2.0)低25-100倍 | 图fb_finesweep |
| KL与维度无关，稳定于~1.5 nats | FB=0.1下18→90维范围KL恒定 | 信息速率不随观测维度增长 | 表4 |
| VQ-VAE通过AWGN实现去噪增益 | 中等SNR(10-20dB)下MSE低于干净信道 | 信道噪声可正则化离散编解码器 | 表5, 图vqvae_channel |
| 统一编解码器优于单场景模型 | spread_xhd(90维)上MSE降5.0% | 跨场景正迁移——共享表示有助于最难任务 | 表6 |
...
**Route B** 将Phase 1基准（单场景，18维）扩展到高维度（最高90维，15个智能体），
为随机编解码器添加系统化的抗坍塌机制，在6种信道损伤模型下测试鲁棒性，
并验证跨场景泛化能力。
...
使用早停和相同的数据分割。Route B总计263个模型，覆盖7个场景、6种智能体数量变体和6种信道损伤模型。
...
<p align="center">
  <img src="assets/rate_distortion.png" width="42%" alt="率失真曲线(Phase 1)">
  <img src="assets/rate_distortion_simple_spread.png" width="42%" alt="率失真曲线(Route B, simple_spread)">
</p>

**左**：Phase 1基准——数字量化主导纯重建；β-VAE在simple_spread(30维)上描绘信息瓶颈前沿。
**右**：Route B以扩展的编解码器配置确认相同结构。
...
<p align="center">
  <img src="assets/stepC_collapse_barrier_full.png" width="82%" alt="坍塌屏障综合图">
</p>
...
<p align="center">
  <img src="assets/stepB_kl_vs_beta.png" width="55%" alt="FB精细扫描分析">
</p>
...
- 重建MSE是一个代理指标；下游策略回报和协调成功率仍需在完整MARL循环中测试（见Phase 3）。
...
│   │   └── adaptive.py           # 码率分配策略
│   ├── data/
│   │   ├── synthetic.py          # 7个场景生成器
│   │   └── __init__.py
│   └── models/
│       ├── pca_baseline.py
│       ├── ae_baseline.py
│       ├── digital_baseline.py
│       ├── vae.py                 # β-VAE + free_bits + 解码器扩展
│       └── vqvae.py               # VQ-VAE + 码本利用率
├── scripts/
│   ├── 0_check_integrity.py
│   ├── 1_collect_data.py
│   ├── 2_train_baselines.py
│   ├── 3_train_vae.py             # β-VAE管线（4阶段）
...
1. 将β-VAE编解码器插入MARL策略循环，在带宽限制下以任务感知失真评估回报。
2. 在训练循环中加入可微信道层（AWGN、瑞利衰落、丢包）用于联合信源信道编码(JSCC)。
3. 评估任务性能指标：协调成功率、碰撞率、路径效率、信道噪声下的通信负载。
4. 测试极限信道条件：SNR ≤ -5dB，丢包率 ≥ 30%。
5. 构建端到端原型：`obs → 编码 → 信道 → 解码 → 策略 → 任务`。

## Route B 完成度: 11/11 (100%)

263个模型，15个结果JSON，17张图表，13个数据集。
