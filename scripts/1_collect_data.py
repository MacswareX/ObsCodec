"""Step 1: Generate synthetic multi-agent observation data for all 7 scenarios.
...
if __name__ == "__main__":
    ...
    # Phase 1: 3 base scenarios (30/24/36 dim)
    print("=" * 60)
    print("Generating Phase 1 scenarios (low-dim)")
    print("=" * 60)
    datasets = collect_synthetic_dataset(save=True)
    for name, data in datasets.items():
        print(f"  {name}: {data.shape}")

    # Step B: 4 high-dim scenarios (40/48/60/90 dim)
    if args.hd or args.xhd or args.all:
        print(f"\n{'=' * 60}")
        print("Generating Step B high-dim scenarios")
        print("=" * 60)
